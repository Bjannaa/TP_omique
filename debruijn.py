...
__author__ = "Bououni Janna"
__copyright__ = "Universite Paris Diderot"
__credits__ = ["Bououni Janna"]
__license__ = "GPL"
__version__ = "1.0.0"
__maintainer__ = "Bououni Janna"
__email__ = "janna1106@yahoo.fr"
...
def select_best_path(
    ...
    best_index = max(
        range(len(path_list)),
        key=lambda i: (weight_avg_list[i], path_length[i])
    )
    return remove_paths(
        graph,
        [path for i, path in enumerate(path_list) if i != best_index],
        delete_entry_node,
        delete_sink_node
    )
...
def solve_entry_tips(graph: DiGraph, starting_nodes: List[str]) -> DiGraph:
    ...
    for start in starting_nodes:
        ...
        if len(successors) > 1:
            paths = [list(all_simple_paths(graph, start, succ)) for succ in successors]
            ...
            graph = select_best_path(graph, all_paths, path_lengths, path_weights, delete_entry_node=True)
    return graph
...
def solve_out_tips(graph: DiGraph, ending_nodes: List[str]) -> DiGraph:
    ...
    for end in ending_nodes:
        ...
        if len(predecessors) > 1:
            paths = [list(all_simple_paths(graph, pred, end)) for pred in predecessors]
            all_paths = [path for sublist in paths for path in sublist]
            path_lengths = [len(path) for path in all_paths]
            path_weights = [path_average_weight(graph, path) for path in all_paths]
            graph = select_best_path(graph, all_paths, path_lengths, path_weights, delete_sink_node=True)
    return graph
